...
export default function WarningsPanel({ warnings, i18n }: Readonly<WarningsPanelProps>) {
    ...
    const panelContent = warnings.map((elem, index) => {
        return (
            <section className="flex flex-col" key={elem.location}>
                <div className="flex items-center">
                    <h3 className="my-4 text-lg font-bold text-primary">{elem.location}</h3>
                </div>
                <Accordion.Root type="single" defaultValue="item-1">
                    {elem.warnings.map((warning, subIndex) => {
                        ...
                        return (
                            ...
                            >
                                <Accordion.Header>
                                    <Accordion.Trigger className="AccordionTrigger w-full text-lg">
                                        <div className="flex items-center">
                                            <div className="flex w-full items-center">
                                                ...
                                                <p className="ml-1 text-left text-sm font-bold text-primary">
                                                    ...
                                                    <span className="block text-xs font-medium opacity-60">
                                                        ...
                                                        {printIssuedByUser(
                                                            warning.meteoalarm_warning_id
                                                        )}
                                                    </span>
                                                </p>
                                            </div>
                                            <ChevronDownIcon
                                                className="AccordionChevron ml-auto size-6"
                                                aria-hidden
                                            />
                                        </div>
                                    </Accordion.Trigger>
                                ...
                                <Accordion.Content>
                                    <div className="mt-2 flex flex-col">
                                        ...
                                        <p className="mt-3 flex items-center border-t border-secondary pt-2 text-xs opacity-60">
                                            <ClockIcon className="mr-1 size-3"></ClockIcon>
    ...
    return <section className="flex size-full flex-col p-3 w-full">{panelContent}</section>;
}
