...
import OffGridStationScene from "@/components/NotFound/OffGridStationScene";
import styles from "@/components/NotFound/NotFound.module.css";

type StationUnavailableProps = {
    lng: string;
    t: (key: string) => string;
    variant?: "page" | "modal";
};

export default function StationUnavailable({
    lng,
    t,
    variant = "page",
}: Readonly<StationUnavailableProps>) {
    const primaryCta = (
        <Link
    ...
    if (variant === "modal") {
        return (
            <div className="flex flex-col items-center gap-4 px-4 py-8 text-center">
                ...
                <p className="max-w-[420px] text-[15px] font-light leading-[1.6] text-[#4a5b66]">
                    {t("body")}
                </p>
                {secondaryCta}
            </div>
