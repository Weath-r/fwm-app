...
>(
    (
        { children, dialogTitle, closeModalButton, scrollable = false, flush = false, ...props },
        forwardedRef
    ) => {
        ...
        return (
            <DialogPrimitive.Portal container={portalContainer}>
                ...
                <DialogPrimitive.Content
                    {...props}
                    ref={forwardedRef}
                    className={`fixed left-1/2 top-1/2 z-10 max-h-[90vh] w-[98vw] max-w-[500px] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white focus:outline-none md:max-h-[98vh] ${
                        flush ? "" : "p-4"
                    } ${dialogClasses}`}
                    ...
                    {dialogTitle && (
                        <DialogPrimitive.Title
                            className={`flex items-center justify-between ${
                                flush ? "sticky top-0 z-10 bg-white px-4 pb-2 pt-3" : "mb-2"
                            }`}
                        >
                            {dialogTitle}
